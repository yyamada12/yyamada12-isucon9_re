...
func main() {
	...
	for _, v := range categories {
		if v.ParentID != 0 {
			category := v
			category.ParentCategoryName = categoryMap[v.ParentID].CategoryName
			categoryMap[v.ID] = category
		}
	}
...
func getNewCategoryItems(w http.ResponseWriter, r *http.Request) {
	...
	rootCategory := categoryMap[rootCategoryID]
...
func getTransactions(w http.ResponseWriter, r *http.Request) {
	...
	qs, args, err := sqlx.In("SELECT te.item_id, te.id, te.status, s.reserve_id FROM `transaction_evidences` te LEFT JOIN `shippings` s ON te.ID = s.transaction_evidence_id WHERE s.`item_id` IN (?) ", itemIDs)
	if err != nil {
		// It's able to ignore Err
		log.Print(err)
	}
	err = tx.Select(&transactionEvidences, qs, args...)
	...
	for _, item := range items {
		...
		if transactionEvidence.ID > 0 {
			ssr, _ := ssrMap.Load(transactionEvidence.ReserveID)
...
func postBuy(w http.ResponseWriter, r *http.Request) {
	...
	category := categoryMap[targetItem.CategoryID]
